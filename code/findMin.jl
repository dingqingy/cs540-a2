...
function findMinL1(funObj,w,lambda;maxIter=100,epsilon=1e-2)
	...
	for i in 1:maxIter
		...
		wNew = w - alpha*g # w^(k+1/2)

		# Proximal step on non-smooth part
		wNew = sign.(wNew).*max.(abs.(wNew) .- lambda*alpha,0) # w^(k+1)
		(fNew,gNew) = funObj(wNew)
...
function proxGradGroupL1(funObj,w,lambda, d, k;maxIter=100,epsilon=1e-2)
	...
	for i in 1:maxIter
		...
		# Decrease the step-size if we increased the function
		gtd = dot(g,wNew-w)
		while fNew + lambda*sum(sqrt.(sum(WNew.^2, dims=2))) > f + lambda*sum(sqrt.(sum(reshape(w,d,k).^2, dims=2))) - gamma*alpha*gtd
			@printf("Backtracking\n")
			alpha /= 2
